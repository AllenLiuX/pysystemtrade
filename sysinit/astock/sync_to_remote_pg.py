...
# Remote Supabase connection (from env)
REMOTE_HOST = 'db.grvepurdpupmjebbcjhp.supabase.co'
REMOTE_PORT = 5432
REMOTE_DB = 'postgres'
REMOTE_USER = 'postgres'
REMOTE_PASS = os.getenv('REMOTE_PG_PASS')
...
def get_remote_conn():
    ...
    if not REMOTE_PASS:
        raise ValueError("REMOTE_PG_PASS not set in environment")
...
def main():
    ...
    print("Sync complete!")
    print("=" * 60)
    print(f"\nRemote DB: postgresql://{REMOTE_USER}:****@{REMOTE_HOST}:{REMOTE_PORT}/{REMOTE_DB}")
